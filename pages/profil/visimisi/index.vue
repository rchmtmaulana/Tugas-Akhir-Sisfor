...
const endpoint = "http://localhost:8055/items/Profil/2"
const visiData = ref([]);

async function getVisi() {
  const api = await fetch(endpoint)
  const data = await api.json()
  console.log(data.data)
  visiData.value = data.data;
}
...
<template>
    ...
    <div class="slug flex w-full text-white" style="height: 120px; margin-top: 130px; width: 100%;">
        ...
        <p style="margin: 0px 0px 0px 5px; padding-top: 30px; font-size: 20px; font-family: Times New Roman Thin; display: flex"> > <span style="margin-left: 5px;" v-html="visiData.nama_halaman"></span></p>
    </div>
    <div  Style="background-color: #d3d3dd">
        <section class="flex justify-center relative" >
            <div class="content offside max-w-screen-xl w-full bg-white border rounded-xl overflow-hidden" style=" margin-top: -30px; padding: 0px 150px 0px 150px;">
                <p class="text-lg" v-html="visiData.konten" style="font-size: 16px;"></p>
            </div>
        </section>
    </div>
    <Footer/>
