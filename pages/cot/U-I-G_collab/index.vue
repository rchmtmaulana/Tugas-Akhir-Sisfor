...
const endpoint = "http://localhost:8055/items/Cot/4"
const uigData = ref([]);

async function getUig() {
  const api = await fetch(endpoint)
  const data = await api.json()
  console.log(data.data)
  uigData.value = data.data;
}
...
<template>
  ...
  <div class="slug flex w-full text-white" style="height: 120px; margin-top: 130px; width: 100%;">
    ...
    <p style="margin: 0px 0px 0px 5px; padding-top: 30px; font-size: 20px; font-family: Times New Roman Thin; display: flex"> > <span style="margin-left: 5px;" v-html="uigData.nama_halaman"></span></p>
  </div>
  <div Style="background-color: #d3d3dd">
      <section class="flex justify-center relative">
          <div class="content offside py-10 px-20 max-w-screen-xl w-full bg-white border rounded-xl">
              <p class="text-lg" v-html="uigData.konten"></p>
          </div>
      </section>
  </div>
  <Footer/>
