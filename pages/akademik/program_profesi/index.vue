...
const endpoint = "http://localhost:8055/items/Akademik/4"
const profesiData = ref([]);

async function getProfesi() {
  const api = await fetch(endpoint)
  const data = await api.json()
  console.log(data.data)
  profesiData.value = data.data;
}
...
<template>
  ...
  <div class="slug flex w-full text-white" style="height: 120px; margin-top: 130px; width: 100%;">
    ...
    <p style="margin: 0px 0px 0px 5px; padding-top: 30px; font-size: 20px; font-family: Times New Roman Thin; display: flex"> > <span style="margin-left: 5px;" v-html="profesiData.nama_halaman"></span></p>
  </div>
  <div Style="background-color: #d3d3dd">
      <section class="flex justify-center relative">
          <div class="content offside py-10 px-20 max-w-screen-xl w-full bg-white border rounded-xl">
              <p class="text-lg" v-html="profesiData.konten"></p>
          </div>
      </section>
  </div>
  <Footer/>
